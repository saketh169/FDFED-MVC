...
exports.adminSignup = async (req, res) => {
  const { name, email, password, adminKey, phone, dob, gender, address } =
    req.body;
...
exports.organizationDashboard = async (req, res) => {
  ...
  try {
    const organization = await Organization.findById(organizationSession.id)
      .select("-password")
      .lean();
    if (!organization) {
      return res
